// --- BEGIN: Functionality moved from november_week_1.js ---
// Nav functions
const navNov = () => {
...
function openModalOne() { if (modalOne) modalOne.style.display = "flex"; }
function openModalTwo() { if (modalTwo) modalTwo.style.display = "flex"; }
function closeModalOne() { if (modalOne) modalOne.style.display = "none"; }
function closeModalTwo() { if (modalTwo) modalTwo.style.display = "none"; }
// Close info modals when clicking outside
window.onclick = function(event) {
    if (event.target === modalOne) closeModalOne();
    if (event.target === modalTwo) closeModalTwo();
};
        ...
        if (query && objectName) {
            ...
            .then(data => {
                if (data?.structured_data) {
                    ...
                    // December circles (if present in structured_data)
                    ...
                    if (data.structured_data.dec_quiz_3 === false) {
                        const quizWheel = document.getElementById('dec-quiz-wheel-3');
...
// --- END: Functionality moved from november_week_1.js ---
